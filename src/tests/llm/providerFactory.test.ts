...
describe('createProvider', () => {
  it('returns OllamaProvider for ollama', () => {
    expect(createProvider({ provider: 'ollama', model: 'llama3.2' })).toBeInstanceOf(
      OllamaProvider
    );
  });
  it('returns OpenAICompatProvider for openai-compatible', () => {
    expect(createProvider({ provider: 'openai-compatible', model: 'gpt-4o' })).toBeInstanceOf(
      OpenAICompatProvider
    );
  });
  it('returns AnthropicProvider for anthropic', () => {
    expect(createProvider({ provider: 'anthropic', model: 'claude-sonnet-4-6' })).toBeInstanceOf(
      AnthropicProvider
    );
  });
