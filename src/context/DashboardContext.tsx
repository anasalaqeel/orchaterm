  ...
  if (raw.llmProviders) {
    return {
      ...
      simpleLlmProvider: raw.simpleLlmProvider ??
        raw.llmProviders.relay ?? { ...DEFAULT_OLLAMA_CONFIG },
...
export const DashboardProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addPipelineTemplate = async (
  ...
  const incrementTemplateUse = async (id: string) => {
    const next = pipelineTemplates.map((t) =>
      t.id === id ? { ...t, useCount: t.useCount + 1, usedAt: new Date().toISOString() } : t
    );
    setPipelineTemplates(next);
    await savePipelineTemplates(next);
  };
