...
export const PipelineLiveBoard: React.FC<PipelineLiveBoardProps> = ({
  plan,
  onDismiss,
  onRerun,
  sessions,
    ...
    return (
      <div className={s.empty}>
        ...
        <p className={s.emptyHint}>
          Build a pipeline in the Builder tab, or describe a goal in Chat — when a plan runs it will
          appear here live.
        </p>
      </div>
    );
  ...
  return (
    <div className={s.root}>
      <div className={s.header}>
        <span className={s.statusIcon} style={{ color: statusColor }}>
          {statusIcon}
        </span>
        <div className={s.headerBody}>
          <div className={s.goalRow}>
            <span className={s.goal} title={plan.goal}>
              {plan.goal}
            </span>
          </div>
          <div className={s.metaRow}>
            ...
            {running > 0 && (
              <span className={s.metaItem} style={{ color: 'var(--color-brand)' }}>
                {running} running
              </span>
            )}
            {done > 0 && (
              <span className={s.metaItem} style={{ color: 'var(--color-success)' }}>
                {done} done
              </span>
            )}
            {failed > 0 && (
              <span className={s.metaItem} style={{ color: 'var(--color-error)' }}>
                {failed} failed
              </span>
            )}
          </div>
        ...
        <div className={s.controls}>
          ...
          {(status === 'done' || status === 'failed' || status === 'stopped') && (
            <>
              ...
              <button title="Dismiss" onClick={onDismiss} className={s.controlBtn}>
                ✕ Dismiss
              </button>
            </>
          )}
      ...
      <div className={s.graphSection}>
        <DependencyGraph
          ...
          title={
            <span>
              Dependency graph · {total} task{total !== 1 ? 's' : ''}
            </span>
          }
        />
      </div>
