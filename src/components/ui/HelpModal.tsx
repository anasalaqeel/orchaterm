    ...
    guides: [
      {
        ...
        steps: [
          ...
          'Click an action pill (e.g. "Status" or "Dev"). Its text is typed into the terminal — or auto-executed if the action has Auto-run enabled.',
          'Prompt-style actions paste without executing, so you can review the text before pressing Enter yourself.',
        ],
      },
      {
        title: 'Prompts with Live Terminal Context',
        badge: 'Variables',
        steps: [
          '(Optional) Highlight any code, error stack trace, or log line in the terminal to populate {{selection}}.',
          'Click a prompt action like "Explain Error" — {{terminal_output}} and friends are expanded with the real text from your terminal just before pasting.',
          'Run it inside a CLI agent terminal (e.g. claude, aider) so the agent receives the full context, or use the right-side AI Chat which already sees your terminal output.',
        ],
      },
      {
        ...
        steps: [
          ...
          'Give the action a label, pick a Lucide icon and color, and decide whether it should auto-run.',
          'Write the command or Markdown prompt. Click context variable chips (e.g. {{selection}}, {{terminal_output}}) to insert dynamic data.',
