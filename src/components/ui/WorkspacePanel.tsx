  ...
  return (
    <div className={s.planWrap}>
      <div className={s.planTop}>
        ...
        <span className={s.planState} style={{ color }}>
          {plan.status}
        </span>
      </div>
      ...
      {plan.tasks
        ...
        .map((t) => (
          <div key={t.id} className={s.runRow}>
            ...
            {t.startedAt && (
              <span className={s.runTime}>{formatDuration(Date.now() - t.startedAt)}</span>
            )}
          </div>
        ))}
    </div>
...
export const WorkspacePanel: React.FC<WorkspacePanelProps> = ({ workspace }) => {
  ...
  useEffect(() => {
    setDraft(workspace.currentTask);
    setEditing(false);
  ...
  const save = () => {
    updateWorkspace(workspace.id, { currentTask: draft });
          ...
          {editing ? (
            ...
            >
              ...
              <div className={s.editRow}>
                ...
                <button className={s.cancelBtn} onClick={cancel}>
                  <X size={10} />
                </button>
              ...
              {recentLogs.map((log, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      style={{
                        color: LOG_STATUS_COLOR[log.status],
                        display: 'flex',
                        flexShrink: 0,
                        marginTop: 1,
                      }}
